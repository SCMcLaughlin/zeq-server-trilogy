...
static uint32_t crc_update(const byte* data, uint32_t len, uint32_t crc)
{
    ...
    for (i = 0; i < len; i++)
    {
        crc = (crc >> 8) ^ s_CrcTable[data[i] ^ (crc & 0xff)];
    }
...
uint16_t crc_calc16(const void* data, uint32_t len, uint32_t key)
{
    ...
    crc = crc_update((const byte*)&key, sizeof(uint32_t), 0xffffffff);
    crc = crc_update((const byte*)data, len, crc);
...
{
    return to_network_uint16(crc_calc16(data, len, key));
}
...
{
    return to_network_uint32(crc_calc32(data, len));
}
