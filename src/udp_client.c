...
static void udpc_report_disconnect(UdpThread* udp, UdpClient* udpc, int zop)
{
    ZPacket zpacket;
    int rc;
...
void udpc_linkdead(UdpThread* udp, UdpClient* udpc)
{
...
bool udpc_send_disconnect(UdpThread* udp, UdpClient* udpc)
{
    typedef struct {
        uint16_t    header;
        uint16_t    seq;
...
void udpc_recv_packet(UdpThread* udp, UdpClient* udpc, Aligned* a, uint16_t opcode)
{
    Aligned w;
    byte* data;
    uint32_t length;
    ...
    udpc_recv_packet_no_copy(udp, udpc, &w, opcode);
}

void udpc_recv_packet_no_copy(UdpThread* udp, UdpClient* udpc, Aligned* a, uint16_t opcode)
{
    ZPacket zpacket;
...
bool udpc_recv_protocol(UdpThread* udp, UdpClient* udpc, byte* data, uint32_t len, bool suppressRecv)
{
    Aligned a;
    uint16_t header;
    uint16_t opcode = 0;
    ...
    */
    ...
    {
        udpc_send_pure_ack(udp, udpc, to_network_uint16(ackRequest));
    ...
    if (!suppressRecv)
    {
        if (ackRequest)
        {
            ...
            ack_recv_packet(udp, udpc, &a, opcode, ackRequest, fragCount);
        }
        else if (opcode)
        {
...
void udpc_send_queued_packets(UdpThread* udp, UdpClient* udpc)
{
